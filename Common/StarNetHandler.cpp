...
void CStarNetHandler::clockInt(unsigned int ms)
{
	...
	if (m_announceTimer.hasExpired()) {
		...
		m_announceTimer.setTimeout(60U * 60U);		// 1 hour
	...
	// Individual user expiry, but not for the permanent entry
	for (CStarNetUsersHashMap::iterator it = m_users.begin(); it != m_users.end(); ++it) {
		CStarNetUser* user = it->second;
		if (user != NULL && !user->getCallsign().IsSameAs(m_permanent))
			user->clock(ms);
	}
	...
	if (m_groupTimer.isRunning() && m_groupTimer.hasExpired()) {
		...
		for (CStarNetUsersHashMap::iterator it = m_users.begin(); it != m_users.end(); ++it) {
			...
			if (user != NULL) {
				if (user->getCallsign().IsSameAs(m_permanent)) {
					permanent = user;
				} else {
					if (m_logFile != NULL) {
						time_t timeNow = ::time(NULL);
		...
		// Re-insert the permenent user
		if (permanent != NULL)
			m_users[m_permanent] = permanent;
