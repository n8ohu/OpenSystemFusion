...
CIRCDDBGatewayConfigFrame::CIRCDDBGatewayConfigFrame(const wxString& title, const wxString& name) :
#else
...
#if defined(__WINDOWS__)
	m_config = new CIRCDDBGatewayConfig(new wxConfig(APPLICATION_NAME), name);
#else
	if (confDir.IsEmpty())
		m_config = new CIRCDDBGatewayConfig(CONF_DIR, CONFIG_FILE_NAME, name);
	else
